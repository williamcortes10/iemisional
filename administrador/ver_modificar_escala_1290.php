...
<html lang="en">
  ...
  <body>
		...
		?>
			...
			<div class="container">
				...
				<div class="panel panel-danger">
				  <div class="panel-heading">Formulario  -  Ver/Modificar Escala 1290</div>
				  <div class="panel-body">
					<form role="form" class="form-horizontal" id='forminstitucion' name='forminstitucion' method='post' action='editar_escala_1290_response.php'>
						<div class="form-group">
							<label for="ie" class="col-sm-2 control-label">AÑO LECTIVO</label>
							<div class="col-sm-3">
							...
							<div class="col-sm-3">
								<button  type="button"  class="btn btn-primary" id="buscar_escala">Buscar  <span class="glyphicon glyphicon-search"></span></button>
							</div>
						</div>
						<div class="busqueda hide">
	...
	$(document).ready(function() {
		$("#buscar_escala").click(function(){
			$.ajax({
				type: "POST",
				dataType: "json",
				url: "consultar_escala_1290_response.php", 
				data: {aniolectivo:$("#aniolectivo").val()},
				success: function(data) {
					$( ".busqueda" ).fadeOut(1000);
					if(data==false){
						$('#myModal .modal-body p').html("No existe escala 1290 para el año lectivo")
						$("#myModal").modal();
					}else{
						
						for(dato in data){
							switch(data[dato].tipo_escala){
								case "DS":	$("#dsmin").val(data[dato].rango_inferior);
											$("#dsmax").val(data[dato].rango_superior);
											break;
								case "DA":	$("#damin").val(data[dato].rango_inferior);
											$("#damax").val(data[dato].rango_superior);
											break;
								case "DB":	$("#dbmin").val(data[dato].rango_inferior);
											$("#dbmax").val(data[dato].rango_superior);
											break;
								case "D-":	$("#dbamin").val(data[dato].rango_inferior);
											$("#dbamax").val(data[dato].rango_superior);
											break;
								default: break;
							}
						}

...
<html>
    <head>
        <title> INGRESAR NOTAS - PLANILLA</title>
	...
	?>
		...
		<table class='tabla'>";
				...
				echo "<td>Nro.</td><td>Estudiante</td><td colspan='1'>COMPETENCIA DE APRENDIZAJE</td><td>V.N</td><td>Faltas J</td><td>Faltas S.J</td><td width='50px'>Comp</td><td>Observaciones</td>";
				echo "</tr>";
				$numero=0; 	
				foreach( $estudiantes as $id){
					if($aniolect<=2014 or $idmateria==27){
							...
							/*$sqlind = "SELECT ib.* FROM indicadoresboletin ib  WHERE ib.iddocente=$docente 
							...
							if($row = $conx->records_array($consulta)){
								...
								while ($rowind = $conx->records_array($consultaind)) {
									...
									echo "<option value='".$rowind['idindicador']."' title='".$rowind['descripcion']."'>".$rowind['idindicador']."</option>";
								...
								echo "<td align='center'><input type='text' id='comp$id' name='comp$id' size='2' maxlength='4' value='3.0'/>";
								...
								<input type='checkbox' id='idestudiante[]' name='idestudiante[]' value='$id' checked >Bloquear escritura</td>
							...
							if($row = $conx->records_array($consulta)){
								...
								while ($rowind = $conx->records_array($consultaind)) {
									echo "<tr>";
									echo "<td style='font-size:10px; background-color:#BDBDBD;  border: 1px dashed #C60;'>".$rowind['competencia']."
									</td><td>
									<select style='width: 85px; float: right;' id='ci$id-".$rowind['consecutivo']."' name='ci$id-".$rowind['consecutivo']."'><option value='F'>Fortaleza</option><option value='D'>Debilidad</option>
									</select></td>";
									echo "</tr>";
								...
								echo "<td align='center' width='300px'><textarea id='obs$id' name='obs$id' cols='30' rows='5' class='toolTip' title='Desactive la casilla para ingresar un aobservación' readonly></textarea>
								<input type='checkbox' id='idestudiante[]' name='idestudiante[]' value='$id' checked >Bloquear escritura</td>
								<input type='hidden' id='listdestudiante[]' name='listdestudiante[]' value='$id'>
								<input type='hidden' id='periodo' name='periodo' value='$periodo'>
								<input type='hidden' id='idmateria' name='idmateria' value='$idmateria'>
								<input type='hidden' id='idaula' name='idaula' value='$idaula'></td>
								<input type='hidden' id='docente' name='docente' value='$docente'></td>
								<input type='hidden' id='aniolect' name='aniolect' value='$aniolect'>";
								echo "</tr>";
							}
							echo "<script>
									$(document).ready(function(){
										...
										$('#comp$id').change(function(e){
											...
											if(isNaN(valor)){
												...
												if(valor<$escalai || valor>$escalas){
													alert('El valor ingresado en el comportamiento debe estar entre 1 y 5');
													$('#comp$id').focus();
													$('#comp$id').val('3.0');

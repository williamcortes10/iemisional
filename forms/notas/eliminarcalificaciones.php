...
<html lang="en">
  <head>
    ...
    <title>SISTEMA ACADEMICO DE CALIFICACIONES - ELIMINAR CALIFICACIONES</title>
  ...
  <body>
		...
		?>
			...
			<?php
				...
				if($conx->get_numRecords($consulta)>0){
					...
					echo "<h3 style='text-align:center;'>ELIMINAR CALIFICACIONES</h3><hr/>";
					...
					<div class="form-group form-inline">
						...
						for($i=1; $i<=$periodoh; $i++){ if($periodoh==$i){echo "<option value='$i' selected>$i °</option>"; }else{echo "<option value='$i'>$i °</option>";}} ?>
						</select>
					...
					while ($row = $conx->records_array($consulta)) {
						...
						$urldel="borrarcalificacionescomp.php?id=$id&idmateria=$idmateria&idaula=$idaula&aniolectivo=$aniolectivo";
						//$urldelnota="borrarcalificacionescompnota.php?id=$id&idmateria=$idmateria&idaula=$idaula&aniolectivo=$aniolectivo";
						echo "<tr>";
						echo "<td>".utf8_encode($row['nombre_materia'])."</td>";
						echo "<td>".utf8_encode($row['descripcion'])."-GRUPO ".$row['grupo']."</td>";
						echo "<td><button type='button' class='btn btn-warning' onclick='borracomp(\"$urldel\")' id='borracomp'>Borrar solo competencias<br/> de los estudiantes</button></td>";
						//echo "<td><button type='button' class='btn btn-danger' onclick='borracompnota(\"$urldelnota\")' id='borracompnota'><span>Borrar competencias y notas<br/> de los estudiantes</button></td>";
						echo "</tr>";
					}
					echo "</tbody>";
					echo "</table>";
	...
	<script>
	function borracomp(url){
		url1=url+"&periodo="+$('#periodo').val();
		if (confirm('ELIMINARA COMPETENCIAS QUE SE INGRESARON A ESTUDIANTES, Realmente desea realizar esta acción?')) {
			$.get(url1, function( data ) {
			  alert(data);
			});
		}
	}
	function borracompnota(url){
		url1=url+"&periodo="+$('#periodo').val();
		if (confirm('ELIMINARA COMPETENCIAS Y NOTAS QUE SE INGRESARON A ESTUDIANTES, Realmente desea realizar esta acción?')) {
			$.get(url1, function( data ) {
			  alert(data);
			});
		}

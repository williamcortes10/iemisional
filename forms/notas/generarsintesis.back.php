		...
		while($rowdinamizador = $conx2->records_array($consultadinamizador)){
				...
				<table class='tabla'>";
						...
						while ($rowindest = $conx2->records_array($consultaindest)) {
							...
							$asignaturas.="<td align='center' title=''>".utf8_encode($rowindest['abreviatura'])."</td>";
							$cadena+=4;
						...
						while ($row2 = $conx2->records_array($consulta)) {
							...
							echo "<td width='300px'>".utf8_encode($row2['apellido1']." ".$row2['apellido2']." ".$row2['nombre1']." ".$row2['nombre2'])."(".$row2['idestudiante'].")</td>";
							...
							while ($rowindest = $conx2->records_array($consultaindest)) {
								...
								if($periodo>=1 and $periodo<=4){
									...
									if($recordsperiodoNV = $conx->records_array($consultarecordNV)){
										if(number_format((float)$recordsperiodoNV['vn'],$numero_decimales,".",",") > number_format((float)$nota,$numero_decimales,".",",") ){
											$nota=number_format((float)$recordsperiodoNV['vn'],$numero_decimales,".",",");
										}
									}
								...
								if ($rownota = $conx2->records_array($consultanota)) {
									...
									if($aniolect<2016){
										...
										if($periodo!='s1' and $periodo!='s2' and $periodo!='F'){
									...
									if($periodo!='s1' and $periodo!='s2' and $periodo!='F'){
										...
										while ($rowind = $conx->records_array($consultaind)) {
											...
											and aniolectivo=$aniolect and periodo=$periodo and idestudiante='$idestudiante' and idmateria=$idmateria";
									...
									//fin verificando nota
										...
										$msjest="background-color:F3E2A9;";
										}elseif($numindest<$numind and $numnota<=0){
											$msjest="background-color:#F2F2F2;";
										}else{
											$msjest="background-color:#white;";
										}
									}
									...
									if($periodo>=1 and $periodo<=4){
										...
										if(round($rownota['vn'], $numero_decimales)<3.0){
											$msjest="background-color:#BDBDBD;";
										}
										echo "<td align='center' style='$msjest'>".number_format(round($rownota['vn'], $numero_decimales), $numero_decimales)."</td>";

<?php
//Datos de Configuración de la apps
session_start();
  ...
  <body>
		...
		?>
			...
			<?php
				...
				AND c.idmateria=m.idmateria AND c.idmateria='49' AND c.aniolectivo='$aniolectivo' AND c.idaula=a.idaula ORDER BY a.grado,a.grupo";
				$consulta = $conx->query($sql);
				if($conx->get_numRecords($consulta)>0){
					...
					while ($row = $conx->records_array($consulta)) {
						...
						echo "<td><a class='btn btn-primary' href='javascript:generarsintesis($id,$idmateria,$idaula,$aniolectivo)'><span>Generar Sintesis</span></a></td>";
						echo "</tr>";
					}
					echo "</tbody>";
					echo "</table>";
				...
				?>
					<div class="jumbotron center-block">
					<h1 class='alert alert-danger'>No puedes generar síntesis porque no es dinamizador de curso.</h1>
					</div>
				<?php
	...
	<script>
		function generarsintesis(id, idmateria, idaula, aniolectivo){
			$(document).ready(function(){
				periodo=$("#periodo").val();
				location.href = "generarsintesis.php?id="+id+"&idmateria="+idmateria+"&idaula="+idaula+

  ...
  <body>
		...
		?>
			...
			<div class="container"><br/><br/>
				...
				<h3>Calificaciones | Regulares | Ingresar Calificación</h3>
				...
				<thead>
					...
					<tr>
						<th colspan='6'>
							Periodo:<select id='periodo' name='periodo' class='form-control' placeholder='Corte ó Periodo'>
							...
							<option value="4">2C Segundo semestre</option></select>    Año lectivo:<select id='aniolectivo' name='aniolectivo' class='form-control' placeholder='Año lectivo'><?php for($i=date("Y"); $i>2010; $i--){?><option value='<?php echo $i; ?>'><?php echo $i; ?></option><?php } ?></select>
						</th>
					</tr>
					<tr>
	...
	$( "#idmateria" ).change(function() {
	  $.post( "lista_docentes.php", { materia: $('#idmateria').val(), aula: $('#idaula').val(),anio: $('#aniolectivo').val(),  periodov: $('#periodo').val()})
		.done(function( data ) {
			$('tbody').html(data);
	  });
	});
	...
	var obtener_data_docente = function(tbody, table){
		$(tbody).on("click","button.calificar", function(){
			...
			window.open("calificacionesxestudiante.php?id="+id+"&idmateria="+idmateria+"&idaula="+idaula+"&aniolectivo="+aniolectivo+"&periodo="+periodo);
		});
	}
	var listar = function() {
		...
		var table = $("#datatable_docentes").DataTable({
			...
			"columns":[
				...
				{"data":"nombre1"},
				{"data":"nombre2"},
				{"defaultContent":"<button class='ver_clases btn btn-info' placeholder='Ingresar calificaciones'><i class='glyphicon glyphicon-edit'></i> Calificar</button>"}
			],

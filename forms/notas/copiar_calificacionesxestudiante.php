...
$aniolectivopost = $_GET['aniolectivopost'];;
$aniolectivoant = $_GET['aniolectivoant'];;
$periodopost = $_GET['periodopost'];;
$periodoant = $_GET['periodoant'];;
$iddocente = $_GET['id'];;
			  ...
			  <div class='container'>
				...
				<div id='navbar' class='collapse navbar-collapse'>
				  <ul class='nav navbar-nav'>
					...
					iddocente='".$_SESSION['k_username']."' AND aniolectivo=$aniolectivopost limit 1";
					$consulta = $conx->query($sql);
					if($conx->get_numRecords($consulta)>0){
					?>
					<li class="dropdown">
...
/*
			...
			$url.="?id=$iddocente&idmateria=$idmateria&idaula=$idaula&aniolectivo=$aniolectivopost";
			echo "<h3 style='text-align:center;'>COPIAR CALIFICACIONES</h3><hr/>";
			...
			?>
				<div class="form-group form-inline">
					...
					if($tipousuario=='A'){
						...
						WHERE c.iddocente='$iddocente' AND c.idmateria=$idmateria AND c.aniolectivo='$aniolectivopost' AND c.idaula=$idaula AND c.periodos LIKE '%$periodo%' LIMIT 1";
						...
						if ($rowp = $conx->records_array($consultap)) {
							...
							foreach($periodos as $valor ){
									if($valor==$periodo){echo "<option value='$valor' selected>$valor °</option>"; }
							}
						}
					}
					$conx->close_conex();
					...
					<input input class="form-control" type='text' id='aniolectivo' name='aniolectivo' value='<?php echo $aniolectivopost; ?>' disabled/>
					<input type='hidden' id='iddocente' name='iddocente' value='<?php echo $iddocente; ?>' />
					<input type='hidden' id='aniolectivo' name='aniolectivo' value='<?php echo $aniolectivopost; ?>' />
					<input type='hidden' id='aniolectivoant' name='aniolectivoant' value='<?php echo $aniolectivoant; ?>' />
					<input type='hidden' id='periodoant' name='periodoant' value='<?php echo $periodoant; ?>' />
    ...
    <script>
		$(document).ready(function(){
				...
				$("#buscar").click(function(){
					...
					aniolectivoant1=$("#aniolectivoant").val();
					periodo1=$("#periodo").val();
					periodoant1=$("#periodoant").val();
					init1=$("#init").val();
					...
					$.post("listaestudiantes_copiar.php",{
						grado: grado1,
						idaula: idaula1,
						idmateria: idmateria1,
						iddocente: iddocente1,
						aniolectivo: aniolectivo1,
						aniolectivoant: aniolectivoant1,
						periodo: periodo1,
						periodoant: periodoant1,
						init: init1,

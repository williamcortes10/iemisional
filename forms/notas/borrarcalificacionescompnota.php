...
if($conx->get_numRecords($consulta)>0){
	...
	while ($row = $conx->records_array($consulta2)) {
		...
		if($row = $conx->records_array($consultaduplicate)){
			$sqlduplicatenota="SELECT DISTINCT * FROM notas WHERE idestudiante ='$id' AND aniolectivo=$aniolectivo AND tipo_nota ='R'
				AND periodo=$periodo AND idmateria=$idmateria";
			$consultaduplicatenota = $conx->query($sqlduplicatenota);
			if($row = $conx->records_array($consultaduplicatenota)){
			...
			while ($rowind = $conx->records_array($consultaind)) {
				...
				$sqlduplicatecomp="SELECT DISTINCT * FROM indicadoresestudiante WHERE idindicador = '$consecutivo' 
					AND idestudiante = '$id' AND aniolectivo = '$aniolectivo' AND periodo = '$periodo'";
	...
	if($numreg==0){
		...
		echo "Se borraron competencias de $numreg registros, $numind competencias borradas";
	}
}else{

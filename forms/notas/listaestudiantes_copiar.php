...
$periodoant = $_POST['periodoant'];
$idmateria = $_POST['idmateria'];
$aniolectivo = $_POST['aniolectivo'];
$aniolectivoant = $_POST['aniolectivoant'];
...
if($numero_competencias_seleccionadas > 0){
			if($aniolectivo<2016){
				$sqlest = "SELECT e.idestudiante, e.apellido1, e.apellido2, e.nombre1, e.nombre2, m.idaula 
					...
					AND c.idmateria=$idmateria AND c.aniolectivo=$aniolectivoant
					AND ib.iddocente=c.iddocente
					AND ib.aniolectivo=c.aniolectivo
					AND ib.periodo=$periodoant
					...
					CONCAT_WS(' ',estudiante.apellido1, estudiante.apellido2, estudiante.nombre1, estudiante.nombre2) AS nombre_estudiante,estudiante.idestudiante AS id, notas.*
					FROM estudiante
					  LEFT JOIN matricula
						ON estudiante.idestudiante = matricula.idestudiante AND estudiante.habilitado='S'
					  LEFT JOIN notas
						ON matricula.idestudiante=notas.idestudiante AND notas.idmateria=$idmateria AND notas.periodo=$periodoant AND notas.aniolectivo=matricula.aniolectivo 
						AND matricula.tipo_matricula=notas.tipo_nota
					WHERE
					  matricula.idaula = $idaula AND matricula.aniolectivo = $aniolectivoant AND matricula.tipo_matricula='R' ORDER BY nombre_estudiante ASC";
			...
			echo "</tbody>";
			echo "</table></div>";
			if($numero_competencias_seleccionadas < 3 or $numero_competencias_seleccionadas > 4){
				...
				while ($row = $conx->records_array($consultaest)) {
					...
					if($row['vn']!=null){
						...
						<?php
						  echo "<input vn_id_estudiante='$id' vn_nota='".$row['vn']."' vn_periodo='$periodo' vn_idmateria='$idmateria' vn_idaula='$idaula' vn_iddocente='$iddocente' vn_grado='$grado' vn_aniolectivo='$aniolectivo' class='vn' vn_id='$id'  vn_escalai='$escalai' vn_escalas='$escalas' data-toggle='tooltip' title='Ingrese valor entre $escalai y $escalas' value='".$row['vn']."' type='text' id='vn$id' name='vn$id' size='2' maxlength='4'/ ><span id='infovn$id'  class='input-group-addon infovn'><i class=\"glyphicon glyphicon-ok text-danger\" style=\"font-size:10px;\"></i></span>"; ?>
						</td>
						</div>
						<td align="left" width="40px">
						<div class="input-group">
						<?php echo "<input fj_id_estudiante='$id' fj_periodo='$periodo' fj_idmateria='$idmateria' fj_idaula='$idaula' fj_iddocente='$iddocente' fj_grado='$grado' fj_aniolectivo='$aniolectivo' class='fj' fj_id='$id' fj_escalai='$escalai' fj_escalas='$escalas' data-toggle='tooltip' title='Faltas justificadas' value='".$row['fj']."' type='text' id='fj$id' name='fj$id' size='2' maxlength='4'/><span id='infofj$id' class='input-group-addon'><i class=\"glyphicon glyphicon-ok text-danger\" style=\"font-size:10px;\"></i></span>"; ?>
						</td>
						</div>
						<td align="left" width="40px">
						<div class="input-group">
						<?php echo "<input fsj_id_estudiante='$id' fsj_periodo='$periodo' fsj_idmateria='$idmateria' fsj_idaula='$idaula' fsj_iddocente='$iddocente' fsj_grado='$grado' fsj_aniolectivo='$aniolectivo' class='fsj' fsj_id='$id' fsj_escalai='$escalai' fsj_escalas='$escalas' data-toggle='tooltip' title='Faltas sin justificar' value='".$row['fsj']."' type='text' id='fsj$id' name='fsj$id' size='2' maxlength='4'/><span id='infofsj$id' class='input-group-addon'><i class=\"glyphicon glyphicon-ok text-danger\" style=\"font-size:10px;\"></i></span>"; ?>
						</td>
						</div>
						<td align="left">

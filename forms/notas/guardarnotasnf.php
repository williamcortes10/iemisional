...
<html>
    <head>
        <title> INGRESAR NOTAS FINALES - PLANILLA</title>
	...
	?>
		...
		if($look and !empty($estudiantes[0])){
				...
				<form id='notas' method='POST' action='guardarnotas_responsenf.php'>
				<table class='tabla'>";
						...
						echo "<tr><td class='estilocelda' colspan='4'>Seleccionar indicadores para boletin final</td></tr>";
						...
						foreach( $estudiantes as $id){
							...
							$sqlind = "SELECT * FROM indicadoresboletinnf ib WHERE ib.iddocente=$docente 
							...
							idindicador NOT IN (SELECT e.idindicador FROM indicadoresestudiantenf e WHERE e.aniolectivo=$aniolect AND e.idestudiante=$id ))";
							...
							if($row = $conx->records_array($consulta)){
								...
								echo "</select>";
								echo "</td>";
								echo "<td align='center'><input type='text' id='comp$id' name='comp$id' size='2' maxlength='4' value='5.0'/></td>";

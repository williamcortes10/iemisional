...
if($formato=="f2"){
	...
	$numasig=0;
	$promtotal=0;	
	//consultando notas de materias de areas militares
	$sqlareas="SELECT DISTINCT codarea, nomarea
	...
	while($recordsnotas = $conx->records_array($consultanotas)){
		$sqlnotas="SELECT DISTINCT codmat, nommat, vn, fj, fsj, ft  FROM `notasmil`, materiamil, areamil 
		...
		while($recordsnotas2 = $conx->records_array($consultanotas2)){
			...
			$fj+=$recordsnotas2['fj'];
			$fsj+=$recordsnotas2['fsj'];			
			$nummat++;
		}
		$prom=$promedio/$nummat;
		$ft=$fsj+$fj;
		if($flagn){
			echo"<tr>
			...
			<td scope='col' class='fonttitle9b' >".$fj."</td>
			<td scope='col' class='fonttitle9b' >".$fsj."</td>
			<td scope='col' class='fonttitle9b' >".$ft."</td>
			</tr>";
	...
	//consultando notas de materias de condiciones militares

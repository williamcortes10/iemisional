  ...
  <body>
		...
		?>
			...
			<?php
				...
				$consulta = $conx->query($sql);
				if($conx->get_numRecords($consulta)>0){
					/* paginacion variable por método GET */
					if (isset($_GET['pag'])){
					 $ini=$_GET['pag'] ;
					}else{
					 $ini=1;
					...
					if($aniolectivo<2016){
						...
						FROM jefearea c, docente d, materia m, aula a
						WHERE d.iddocente='$iddocente' AND c.iddocente=d.iddocente 
						AND c.idmateria=m.idmateria AND c.aniolectivo='$aniolectivo' AND a.idaula=c.idaula 
						ORDER BY a.grado,a.grupo
						LIMIT $init, $limit_end";
					}	
					...
					//echo "<caption><h3 style='text-align:center;'>ACTUALIZAR O ELIMINAR COMPETENCIAS<br/> $corte</h3></caption>";
					echo "<caption><h3 style='text-align:center;'>ACTUALIZAR O ELIMINAR COMPETENCIAS</h3></caption>";
					echo "<thead>";
					echo "<tr>";
					...
					while ($row = $conx->records_array($consulta)) {
						...
						echo "<td><a class='btn btn-info' href='listarcompetencias.php?id=$id&idmateria=$idmateria&idaula=$idaula&aniolectivo=$aniolectivo'><span>Ver competencias</span></a></td>";
						echo "</tr>";
					}
					echo "</tbody>";
					echo "</table>";

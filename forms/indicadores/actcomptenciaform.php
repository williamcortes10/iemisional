...
$sql = "SELECT * FROM plan_curricular WHERE consecutivo = '$consecutivo'
and iddocente = '$iddocente'  ORDER BY consecutivo DESC";
$consulta = $conx->query($sql);
while ($row = $conx->records_array($consulta)) {
		$estbc = $row['estandarbc'];
		$competencia = $row['competencia'];
		$grados = $row['grados'];
		$grado = $row['gradounico'];
}
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <body>
		...
		?>
			...
			<div class="forminsert center-block">
				<h3>Actualizar competencia en <?php echo utf8_decode($nommateria); ?></h3>
				<hr/>
				<form class="form-signin" role="form" action="actcomptenciaformsucess.php" method='POST' id='frmnvcomp' name='frmnvcomp'>
				<div class="form-group form-inline">
					<label for="consecutivo">Codigo competencia:</label>
					<input type="text" class="form-control" id="consecutivo" name="consecutivo" value='<?php echo $consecutivo; ?>' disabled>
				</div>
				<div class="form-group form-inline">
			    ...
			    <select id='estbc' name='estbc' class="form-control">
						...
						if($conx->get_numRecords($consulta)>0){
							while ($row = $conx->records_array($consulta)) {
								if($estbc==$row["codigo"]){
									echo "<option value='".$row["codigo"]."' selected>".
			    ...
			    <textarea class="form-control" rows="6" id='desestbc' name='desestbc' required autofocus><?php echo utf8_encode($competencia); ?></textarea>
				</div>
				<?php
					$grgrados=explode("-",$grados);
					if($grgrados){
						$grgradoi=$grgrados[0];
						$grgradof=$grgrados[1];
					}else{
				...
				<div class="form-group form-inline">
					...
					<?php for($i=1; $i<12; $i++){ if($grgradoi==$i){echo "<option value='$i' selected>$i °</option>"; }else{echo "<option value='$i'>$i °</option>";}} ?>
					</select> 
					<label for="grgradof">hasta</label>
					<select id='grgradof' name='grgradof' class="form-control">
					<?php for($i=1; $i<12; $i++){ if($grgradof==$i){echo "<option value='$i' selected>$i °</option>"; }else{echo "<option value='$i'>$i °</option>";}} ?>
					</select> 
				 </div>

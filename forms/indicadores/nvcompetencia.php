...
<html lang="en">
  <head>
    ...
    <title>SISTEMA ACADEMICO DE CALIFICACIONES - NUEVA COMPETENCIA</title>
  ...
  <body>
		...
		?>
			...
			<?php
				/*switch($periodo){
					case 1: $corte="PRIMER CORTE - PRIMER SEMESTRE"; break;
					case 2: $corte="SEGUNDO CORTE - PRIMER SEMESTRE"; break;
					case 3: $corte="PRIMER CORTE - SEGUNDO SEMESTRE"; break;
					case 4: $corte="SEGUNDO CORTE - SEGUNDO SEMESTRE"; break;
					default: $corte=""; break;
				}
				...
				AND c.idmateria=m.idmateria AND c.aniolectivo='$aniolectivo' AND c.idaula=a.idaula 
				AND (c.periodoinicial<=$periodo AND c.periodofinal>=$periodo) ORDER BY a.grado,a.grupo";*/
				if($aniolectivo<2016){
					$sql = "SELECT c.iddocente, m.idmateria, m.nombre_materia, c.aniolectivo, a.idaula, a.descripcion, a.grupo
					FROM clase c, docente d, materia m, aula a
					WHERE d.iddocente='$iddocente' AND c.iddocente=d.iddocente 
				...
				if($conx->get_numRecords($consulta)>0){
					...
					//echo "<caption><h3 style='text-align:center;'>INTENSIDAD HORARIA AÑO LECTIVO $aniolectivo<br/> $corte</h3></caption>";
					if($aniolectivo<2016){
					...
					while ($row = $conx->records_array($consulta)) {
						...
						//echo "<td>".utf8_encode($row['descripcion'])."-GRUPO ".$row['grupo']."</td>";
						echo "<td>".($row['descripcion'])."</td>";
						echo "<td><a class='btn btn-primary' href='nvcomptenciaform.php?id=$id&idmateria=$idmateria&idaula=$idaula&aniolectivo=$aniolectivo'><span>Crear competencias</span></a></td>";
						echo "</tr>";
					}
					echo "</tbody>";
					echo "</table>";
			...
			}else if($tipousuario=='A')
			{
				...
				if($conx->get_numRecords($consulta)>0){					
					...
					<h3 class='text-left'>Competencias académicas | Nueva</h3>
						<div class="panel panel-danger">
						  <div class="panel-heading">
							<h3 class="panel-title">
								<?php
									if($aniolectivo<2016){
								?>
									INTENSIDAD HORARIA - AÑO LECTIVO <?php echo $aniolectivo; ?>
								<?php
									}else{
								?>
									COORDINADORES DE AREA - AÑO LECTIVO <?php echo $aniolectivo; ?>
								<?php
									}
								?>
							</h3>
						  ...
						  <div class="panel-body">
							<table cellpadding="10" id="datatable" class="display table-striped table-responsive table-responsive" cellspacing="0" width="100%">
								<thead>
									<tr>
										<th>Asignatura</th>
									...
									<?php
										while ($row = $conx->records_array($consulta)) {
											...
											<tr>
												...
												<td><a class='btn btn-primary' <?php echo "href='nvcomptenciaform.php?id=$id&idmateria=$idmateria&idaula=$idaula&aniolectivo=$aniolectivo'"; ?> ><span>Crear competencias</span></a></td>
											</tr>
											<?php
										}
									?>
